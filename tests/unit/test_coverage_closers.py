...
# -- sigreen credential resolver: config load failure falls back to env (lines 36-37) --


def test_sigreen_client_id_falls_back_to_env_when_config_unreadable(monkeypatch) -> None:
    from app.integrations import sigreen as sigreen_mod

    monkeypatch.delenv("SIGREEN_CLIENT_ID", raising=False)
    monkeypatch.setenv("SIGREEN_CLIENT_ID", "env-client")
    with patch("app.services.config_service.load_app_config", side_effect=OSError("no config")):
        assert sigreen_mod._sigreen_client_id() == "env-client"
