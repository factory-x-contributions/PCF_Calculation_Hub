...
def test_compressed_air_type_with_space_spelling(constant_emission_factor) -> None:
    """``compressed air`` (two words) must use the compressed-air conversion path."""
    breakdown, kwh, cf, _ = calculate_energy_cf(
        _make_consumption(_energy("compressed air", "kWh", 4.0))
    )
    assert len(breakdown) == 1
    entry = next(iter(breakdown.values()))
    assert entry["uom"] == "kWh"
    assert entry["total_consumption"] == pytest.approx(4.0)
    assert kwh == pytest.approx(4.0 * COMPRESSED_AIR_KWH_PER_NM3)
    assert cf == pytest.approx(4.0 * COMPRESSED_AIR_KWH_PER_NM3 * 350 / 1000)
