...
def test_create_own_emission_cf_report_passes_idle_and_empty_batch() -> None:
    """``producedMtus`` empty → ``batch_number=None``; idle CF merged into the report builder."""
    data = ProductionResults.model_validate({**http_payloads.prod_result_1, "producedMtus": []})
    fake_sigi = MagicMock()
    with patch("app.services.pcf_service._get_sigreen", return_value=fake_sigi):
        with patch(
            "app.services.pcf_service.collect_idle_cf_by_machine_for_work_order",
            return_value={"Idle: M1": 0.5},
        ) as idle_mock:
            with patch(
                "app.services.pcf_service.build_pcf_report",
                return_value={"productCarbonFootprint": 12.0},
            ) as build_mock:
                with patch("app.services.pcf_service.carbon_intensity_gco2_per_kwh", return_value=350.0):
                    out = pcf_service.create_own_emission_cf_report(
                        {"OP10": 10.0}, data, materials_breakdown=None
                    )
    idle_mock.assert_called_once()
    assert build_mock.call_args.kwargs["batch_number"] is None
    assert build_mock.call_args.kwargs["idle_by_activity"] == {"Idle: M1": 0.5}
    assert out["productCarbonFootprint"] == 12.0
