...
def test_router_logs_exception_before_reraising(client_with_fake_use_case, caplog) -> None:
    """The router's ``except`` block must log then re-raise (lines 56–58 in consumption.py)."""
    import logging

    client, use_case = client_with_fake_use_case
    use_case.execute.side_effect = ValueError("bad payload state")
    with caplog.at_level(logging.ERROR, logger="pcf_creator_app"):
        response = client.post("/consumptionData", json=http_payloads.op_1)
    assert response.status_code == 500
    assert any("Error processing consumption" in r.message for r in caplog.records)
