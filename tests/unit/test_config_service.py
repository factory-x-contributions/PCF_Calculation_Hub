...
def test_ensure_sigreen_factory_id_swallows_constructor_errors(isolated_config_path) -> None:
    config_service.save_app_config({"pcf_tool": "sigreen", "sigreen_factory_name": "OPC"})
    with patch(
        "app.integrations.sigreen.SiGREENInterface",
        side_effect=RuntimeError("network down"),
    ):
        config_service.ensure_sigreen_factory_id()
    assert config_service.load_app_config().get("sigreen_factory_id", "") == ""


def test_ensure_sigreen_factory_id_clears_id_when_factory_not_found(isolated_config_path) -> None:
    config_service.save_app_config(
        {"pcf_tool": "sigreen", "sigreen_factory_name": "Missing", "sigreen_factory_id": "stale"}
    )
    fake_sigi = MagicMock(factory_id=None)
    with patch("app.integrations.sigreen.SiGREENInterface", return_value=fake_sigi):
        config_service.ensure_sigreen_factory_id()
    assert config_service.load_app_config()["sigreen_factory_id"] == ""


def test_infer_sigreen_audience_empty_base_url_uses_default() -> None:
    assert config_service.infer_sigreen_audience_from_base_url("") == config_service.DEFAULT_SIGREEN_AUDIENCE


def test_infer_sigreen_audience_invalid_url_uses_default() -> None:
    assert config_service.infer_sigreen_audience_from_base_url("not-a-url") == config_service.DEFAULT_SIGREEN_AUDIENCE
